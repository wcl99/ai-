...
- 平台在终态同步时会提取小易工具结果中明确存在的 `vuln_info`，幂等写入平台漏洞记录；不会根据普通日志或模型描述推断漏洞。
- 小易未提供 `reportUrl` 时，平台会使用已回传的结构化证据生成本地 Markdown 兜底报告，并沿用现有预览、下载和审计接口。
- 上游返回失败但已有成功工具结果时，平台状态为 `PARTIAL_SUCCEEDED`，同时保留失败工具和未完成原因；没有有效执行证据时仍为 `FAILED`。
